...
export const SectionHeader = ({
  ...
  return (
    ...
    { actions.map((action, index) => <SectionHeaderAction key={index} icon={action.icon} text={action.text} />)}
      </Col>
    </Row>
  );
};

const SectionHeaderAction = ({ icon, text }) => {
  return (
    <Row align={"center"}>
      <Col size={1} justify="flex-end">
        {icon}
        <Text type="subheading" margin={"0 10px"}>{text}</Text>
      </Col>
      {/* <Col size={3}>
      </Col> */}
    </Row>
